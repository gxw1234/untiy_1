...
namespace Client.MirScenes
{
    public sealed class LoginScene : MirScene
    {
        ...
        public LoginScene()
        {
            ...
            _background.Location = new Point((Settings.ScreenWidth - _background.Size.Width) / 2, (Settings.ScreenHeight - _background.Size.Height) / 2);
            ...
            _login.ViewKeyButton.Click += (o, e) =>     //ADD
            {
                ...
                _ViewKey.Location = new Point((_background.Size.Width - _ViewKey.Size.Width) / 2 + 285, (_background.Size.Height - _ViewKey.Size.Height) / 2 + 150);
            };
        ...
        private void SendVersion()
        {
            ...
            {
                byte[] sum;
                sum = Encoding.ASCII.GetBytes("测试");
                p.VersionHash = sum;
                Network.Enqueue(p);
            }
            catch (Exception ex)
        ...
        private void ClientVersion(S.ClientVersion p)
        {
            switch (p.Result)
            {
                ...
                case 1:
                    ...
                    //Network.Enqueue(new C.Login {AccountID = "admin", Password = "admin"});
                    break;
            }
        }
        ...
        public sealed class LoginDialog : MirImageControl
        {
            ...
            public LoginDialog()
            {
                Index = 1084;
                Library = Libraries.Prguse;
                Size = Libraries.Prguse.GetSize(Index);
