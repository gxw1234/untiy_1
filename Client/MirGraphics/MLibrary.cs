...
namespace Client.MirGraphics
{
    ...
    public sealed class MLibrary
    {
        ...
        public void Draw(int index, Point point, Size size, Color colour)
        {
            ...
            UnityEngine.Matrix4x4 scaling = UnityEngine.Matrix4x4.Scale(new UnityEngine.Vector3(scaleX, scaleY, 1f));
            UnityEngine.Matrix4x4 transformMatrix = scaling;
            UnityEngine.GL.MultMatrix(transformMatrix);
        ...
        public bool VisiblePixel(int index, Point point,Point FinalDrawLocation)
        {
            if (!CheckImage(index))
                return false;

            int w = _images[index].Width;
            int h = _images[index].Height;

            Rectangle objectRect = new Rectangle(FinalDrawLocation.X,FinalDrawLocation.Y,w,h);  

            // 检查点击坐标是否在物体范围内
            if(objectRect.Contains(point))
            {
                return true;
            }
    ...
    public sealed class MImage
    {
        ...
        public UnityEngine.Texture2D MaskImage;
        ...
        public unsafe void CreateTexture(BinaryReader reader)
        {
            ...
            Image = new UnityEngine.Texture2D(w, h, UnityEngine.TextureFormat.ARGB32, true);
            ...
            if (HasMask)
            {
                ...
                MaskImage = new UnityEngine.Texture2D(w, h, UnityEngine.TextureFormat.ARGB32, true);
        ...
        private UnityEngine.Color32[] covert(byte[] decomp, int width, int heigth)
        {
            int length = decomp.Length / 4;
            UnityEngine.Color32[] resout = new UnityEngine.Color32[length];
            int index = 0;
            //行数
            for (int h = heigth - 1; h >= 0; h--)
            {
                ...
                {
                    int j = w * 4 + (h * 4 * width);
                    resout[index++] = new UnityEngine.Color32(decomp[j + 2], decomp[j + 1], decomp[j], decomp[j + 3]);
                }
            }
            return resout;
        }
        ...
        public unsafe void DisposeTexture()
        {
            ...
            if (MaskImage != null)
            {
                UnityEngine.Object.Destroy(MaskImage);
            }
