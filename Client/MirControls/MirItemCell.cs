            ...
            if (Item != null && GameScene.SelectedCell != this && Locked != true)
            {
                ...
                if (Library != null)
                {
                    ushort image = Item.Image;
                       int Index = Item.Image;
                            int itemnumber = ItemSlot;
                    ...
                    if (GridType == MirGridType.Craft)
                    {
                        Libraries.Prguse.Draw(1121, DisplayLocation.Add(new Point(-2, -1)), Color.White, UseOffSet, 0.8F);
                           ...
                           if(ItemSlot<6){
                            ...
                            FairyGUI.GList bagItemCellList = GameScene.Scene.bagItemCell.asList;  
                            bagItemCellList.itemRenderer = GameScene.Scene.RenderInventoryItem;
                            GameScene.Scene.RenderInventoryItem(Index, bagItemCellList.GetChildAt(itemnumber));
                        }
        ...
        private void DisposeCountLabel()
{
    ...
    if (CountLabel != null && !CountLabel.IsDisposed)
    {
        ...
        CountLabel.Dispose();
        CountLabel = null; // 处理完后将 CountLabel 设为 null
        
    }
    if(ItemSlot<6){
     FairyGUI.GList bagItemCellList = GameScene.Scene.bagItemCell.asList;  
        bagItemCellList.itemRenderer = GameScene.Scene.RenderInventoryItem;
        GameScene.Scene.RenderInventoryItem(-1, bagItemCellList.GetChildAt(ItemSlot));
    }
}
