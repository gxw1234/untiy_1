...
namespace Client.MirScenes.Dialogs
{
    public sealed class MainDialog : MirImageControl
    {
        ...
        public MainDialog()
        {
            ...
            GameScene.Scene.DBtnBag.onClick.Add((_) =>  
           {  
            ...
            //角色面板
             GameScene.Scene.DBtnState.onClick.Add((_) =>  
           {  
            ...
            GameScene.Scene.DBtnShop.onClick.Add((_) =>  
           {  
        ...
        public void Process()
        {
            ...
            if (Settings.HPView)
            {
                ...
                GameScene.Scene.Hptitle.text = string.Format("HP {0}/{1}", User.HP, User.Stats[Stat.HP]);
                ManaLabel.Text = HPOnly ? "" : string.Format("MP {0}/{1} ", User.MP, User.Stats[Stat.MP]);
                GameScene.Scene.Mptitle.text = HPOnly ? "" : string.Format("MP {0}/{1} ", User.MP, User.Stats[Stat.MP]);
            ...
            else
            {
                ...
                GameScene.Scene.Hptitle.text = string.Empty;
                ManaLabel.Text = string.Empty;
                GameScene.Scene.Mptitle.text = string.Empty;
            }
             GameScene.Scene.MpWin1.value =100 * (double)((double)User.MP / (double)User.Stats[Stat.MP]);
             GameScene.Scene.HpWin1.value =100 * (double)((double)User.HP / (double)User.Stats[Stat.HP]);
            GameScene.Scene.DLevelText.text = User.Level.ToString();
            LevelLabel.Text = User.Level.ToString();
            ExperienceLabel.Text = string.Format("{0:#0.##%}", User.Experience / (double)User.MaxExperience);
            GameScene.Scene.DExpBar.value = User.Experience / (double)User.MaxExperience*100;
            ...
            GameScene.Scene.DWetBar.value = (double)MapObject.User.CurrentBagWeight / (double)MapObject.User.Stats[Stat.BagWeight]*100;
        }
    ...
    public sealed class ChatDialog : MirImageControl
    {
        ...
        public Font ChatFont = new Font(Settings.FontName, 12F);
        ...
        public void ReceiveChat(string text, ChatType type)
        {
            ...
            for (int i = 1; i < text.Length; i++)
            {
                ...
                UnityEngine.Vector2 textSize = CMain.style.CalcSize(new UnityEngine.GUIContent(text.Substring(index, i - index)));
                if (textSize.x > chatWidth)
                {
                    int offset = i - index;
                    int newIndex = i - 1;
         ...
         public static UnityEngine.Color ToUnityColor(Shared.Unity.Color sharedColor)  
        {  
            return new UnityEngine.Color(sharedColor.R / 255f, sharedColor.G / 255f, sharedColor.B / 255f, sharedColor.A / 255f);  
        }  
...
//     FairyGUI.GImage background = item.GetChild("ChatWindowBG").asImage;

//     // 获取对应的历史记录
//     ChatHistory history = History[index]; // 注意这里的索引是从0开始的

//     // 更新文本和背景颜色
//     textField.text = history.Text;
//     background.color=ColorConverter.ToUnityColor(history.BackColour);
...
// }
        public void Update()
        {
            ...
            for (int i = StartIndex; i < History.Count; i++)
            {
                ...
                FairyGUI.GImage chatWindowBG = new FairyGUI.GImage(); 
                 GameScene.Scene.DChatWindow.numItems = History.Count;
                //  chatWindowBG.color = ColorConverter.ToUnityColor(History[i].BackColour); 
                //  FairyGUI.GRichTextField ChatWindowText = new FairyGUI.GRichTextField(); 
                //  GameScene.Scene.DChatWindow.AddChild(chatWindowBG);
                // ChatWindowText.text= History[i].Text;
                //    GameScene.Scene.DChatWindow.AddChild(ChatWindowText);
                ...
                foreach (Match match in RegexFunctions.ChatItemLinks.Matches(currentLine).Cast<Match>().OrderBy(o => o.Index).ToList())
                {
                    try
                    {
                        ...
                        Size size = CMain.GetTextSize(text, temp.Size, temp.Font);
    ...
    public sealed class SkillBarDialog : MirImageControl
    {
        ...
        public SkillBarDialog()
        {
            ...
            for (var i = 0; i < Cells.Length; i++)
            {
                ...
                int j = i + 1;
                     ...
                     GameScene.Scene.buttons[i].onClick.Add((_) =>  
           {  
                GameScene.Scene.UseSpell(j + (8 * BarIndex));
            }); 
                   
                        GameScene.Scene.buttons[i+8].onClick.Add((_) =>  
           {  
                GameScene.Scene.UseSpell(j + (8 * BarIndex));
            }); 
        ...
        public void Update()
        {
            ...
            for (var i = 1; i <= 8; i++)
            {
                Cells[i - 1].Index = -1;
                 if(BarIndex == 0){
                    GameScene.Scene.Skillloade(i-1, -1);
                    }
                    else{
                        GameScene.Scene.Skillloade(i+7, -1);
                        }
                ...
                foreach (var m in GameScene.User.Magics)
                {
                    ...
                    if(BarIndex == 0){
                    GameScene.Scene.Skillloade(i-1, magic.Icon * 2);
                    }
                    else{
                        GameScene.Scene.Skillloade(i+7, magic.Icon * 2);
                        }
    ...
    public sealed class MiniMapDialog : MirImageControl
    {
        ...
        private void MiniMap_BeforeDraw(object sender, EventArgs e)
        {
            ...
            int mapIndex = map.MiniMap;
            UnityEngine.Texture2D miniMapTexture = Libraries.MiniMap.GetTexture(mapIndex);
            UnityEngine.Texture2D miniMapTexture1 = miniMapTexture;
            FairyGUI.NTexture mapTexture = new FairyGUI.NTexture(miniMapTexture);
            UnityEngine.Rect viewRect1 = viewRect.ToRect1(miniMapSize.Height);
             UnityEngine.Texture2D croppedTexture = GameScene.Scene.CropTexture(miniMapTexture1, viewRect1, scaleX, scaleY);
             //mapTexture.Dispose();
              mapTexture = new FairyGUI.NTexture(croppedTexture);
              FairyGUI.GLoader minimapLoader = GameScene.Scene.DMap.GetChild("item").asLoader;
            minimapLoader.texture = mapTexture;
