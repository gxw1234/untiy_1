...
namespace Client.MirScenes
{
    public sealed class GameScene : MirScene
    {
        ...
        public RockingBar rockingBarRun;//跑步
        public RockingBar rockingBarWalk;//走路
        public long JoystickMovetime;//触摸事件
        public GComponent DJoystick;
        public GComponent DoubleDJoystick;
        public GComponent DMap;
        public GLoader DRun;
        public GButton DBtnShop;
        public GButton DBtnShow;
        public GButton DBtnState;
        public GButton DBtnBag;
        public GButton DA2ESimpleButton1;
        public bool IsDRun = true;
        public MirDirection mirDirection;
        public bool isMoveRocking = false;
        public GProgressBar DExpBar; 
        public GProgressBar DWetBar; 
        public GTextField DLevel;
         public GProgressBar HpWin1; 
         public GProgressBar MpWin1;
          public GTextField DLevelText;
         public GTextField Hptitle;
         public GTextField Mptitle;
          public GList DChatWindow;
          public GList bagItemCell;
          public GButton[] buttons;
          public GLoader iconLoader;
         public GButton ChangeSkill;
          public GLoader[] Skill;
          public GImage ChatWindowBG;
          public GRichTextField ChatWindowText;
          
    bool isAnimationPlaying = false;  
    bool isPlayingForward = true;
        bool isAnimationPlaying1 = false;  
    bool isPlayingForward1 = true;
     bool isAnimationPlaying2 = false;  
    bool isPlayingForward2 = true;
      bool isAnimationPlaying3 = false;  
    bool isPlayingForward3 = true;
    public FairyGUI.NTexture itemtexture;
    public FairyGUI.NTexture Skilltexture;

 private bool isItemInfoVisible = false; // 用于跟踪物品信息的可见性
public UnityEngine.Texture2D CropTexture(UnityEngine.Texture2D texture, UnityEngine.Rect rect, float scaleX, float scaleY)
{
    if (texture == null)  
{  
    UnityEngine.Debug.LogError("Texture is null. Cannot crop a null texture.");  
    return null; // 或者返回一个默认的纹理或其他适当的处理方式  
}
    int croppedWidth = (int)(rect.width * scaleX);
     int croppedHeight = (int)(rect.height * scaleY);
    UnityEngine.Texture2D croppedTexture = new UnityEngine.Texture2D(croppedWidth, croppedHeight);
    
    for (int x = 0; x < croppedWidth; x++)
    {
        for (int y = 0; y < croppedHeight; y++)
        {
            int srcX = (int)((x / scaleX) + rect.x);
           int srcY = (int)((y / scaleY) + rect.y);

            if (srcX >= 0 && srcX < texture.width && srcY >= 0 && srcY < texture.height)
            {
                 croppedTexture.SetPixel(x, y, texture.GetPixel(srcX, srcY));
            }
        }
    }
...
public void Skillloade(int index ,int index2)  
{  
    
    // 确保 obj 不为空
   if ( index == -1 || index2 == -1 )
    {   GLoader skil = buttons[index].GetChild("skill").asLoader;
         Skilltexture = null;  
        skil.texture = Skilltexture;  
         skil.icon = null; 
 
        //UnityEngine.Debug.LogError($"RenderInventoryItem: Texture not found for index {index}");
        return;
    }
    else{
   
    GLoader skil = buttons[index].GetChild("skill").asLoader;
    UnityEngine.Texture2D newTexture = Libraries.MagIcon.GetTexture(index2);
    FairyGUI.NTexture skilltexture = new FairyGUI.NTexture(newTexture);
...
public void RenderInventoryItem(int index, FairyGUI.GObject obj)  
{  
    // 确保 obj 不为空
    if (obj == null)
    {
        UnityEngine.Debug.LogError("RenderInventoryItem: obj is null");
        return;
    }

    FairyGUI.GComponent item = obj.asCom;  
    
    // // 确保 item 不为空
    // if (item == null)
    // {
    //     iconLoader = item.GetChild("Kicon").asLoader;  
    //     iconLoader.icon = null; 
    //     //UnityEngine.Debug.LogError("RenderInventoryItem: item is null");
    //     return;
    // }
    ...
    // 获取物品的纹理  
    UnityEngine.Texture2D newTexture = Libraries.Items.GetTexture(index);

    // 检查纹理是否有效
    if (newTexture == null || index == -1 || item == null)
    {   
         itemtexture = null;  
        iconLoader.texture = itemtexture;  
         iconLoader.icon = null; 
    ...
    iconLoader.SetSize(32, 32);
    
    // 更新物品图标  
    itemtexture = new NTexture(newTexture);  
    iconLoader.texture = itemtexture;  
    ...
    // 添加点击事件监听  
    item.onClick.Add(() => OnItemClick(index));  
    }
...
{
   UnityEngine.Debug.Log($"物品 {index} 被点击了");
}
        ...
        void RenderListItem(int index, FairyGUI.GObject obj)
{
    ...
    // 获取文本和背景组件
    FairyGUI.GTextField textField = item.GetChild("ChatWindowText").asTextField;
    FairyGUI.GGraph background = item.GetChild("ChatWindowBG").asGraph;

    // 获取对应的历史记录
    ChatDialog.ChatHistory history = ChatDialog.History[index]; // 注意这里的索引是从0开始的

    // 更新文本和背景颜色
    textField.text = history.Text;
    textField.color = history.ForeColour.ToUnity();
   if (background == null)  
{  
    UnityEngine.Debug.LogError("Background component is null!");  
    return;  
}  
  
if (history == null)  
{  
    UnityEngine.Debug.LogError("History item is null!");  
    return;  
}  
  
// 假设 BackColour 是一个 Color 结构体，它不会是 null，但我们应该确保 history 是有效的  
try  
{  
    background.color = history.BackColour.ToUnity();  
}  
catch (Exception ex)  
{  
    UnityEngine.Debug.LogError("Failed to set background color: " + ex.Message);  
    // 可以在这里添加更多的错误处理逻辑  
}

    // 添加点击事件监听器（如果需要）
    // textField.onClick.Add(() =>
    // {
    //     // 处理点击事件
    //     // ...
    // });
        ...
        {
            JoystickMovetime = CMain.Time;
            
            GComponent TMP = UIPackage.CreateObject("UI","主界面_MainUI").asCom;
            GComponent TaskAndRanksPanel = TMP.GetChild("TaskAndRanksPanel").asCom;
            GComponent DFunBtms = TMP.GetChild("DFunBtms").asCom;
            GComponent DArrackModelUI = TMP.GetChild("DArrackModelUI").asCom;
            //技能按钮
             buttons = new GButton[16];  
            for (int i = 0; i < 16; i++)  
            {  
             
            string buttonName = "DBtnAttack" + (i + 1);  
            buttons[i] = DArrackModelUI.GetChild(buttonName).asButton;  
            //Skill[i]= buttons[i].GetChild("skill").asLoader;  
            } 
            //钓鱼切换
            GButton ChangeSKILL = DArrackModelUI.GetChild("ChangeSKILL").asButton;
            if (ChangeSKILL != null)  
            {  
                ChangeSKILL.onClick.Add((_) =>  
                {  
                    if (!isAnimationPlaying3)  
                    {  
                        isAnimationPlaying3 = true;  
                        ChangeSKILL.enabled = false; // 禁用按钮  
                        DBtnShow.enabled = false;
                        ChangeSkill.enabled = false; 
                        Transition trans = DArrackModelUI.GetTransition("ChangeSKILL");  
                        ...
                        {  
                            ...
                            {  
                                ...
                                // 动画完成后重新启用按钮  
                                isAnimationPlaying3 = false;  
                                ChangeSKILL.enabled = true; 
                                
                                 
                            };  

                            PlayCompleteCallback onComplete1 = () =>  
                            {  
                                 DBtnShow.enabled = true; 
                                 ChangeSkill.enabled = true; 
                                // 动画完成后重新启用按钮  
                                isAnimationPlaying3 = false;  
                                ChangeSKILL.enabled = true; 
                        ...
                        if (isPlayingForward3)  
                        {  
                            trans.Play(onComplete);  
                             
                        }  
                        else  
                        {  
                            // 假设PlayReverse也可以接受回调，如果不是，需要另外处理  
                            trans.PlayReverse(onComplete1); 

                           
 
                        }  
                        isPlayingForward3 = !isPlayingForward3;  
                        }  
                    }  
                });  
            }  
            ...
            {  
                UnityEngine.Debug.LogError("DA2ESimpleButton1 is null");  
            }
            //技能切换
            ChangeSkill = DArrackModelUI.GetChild("ChangeSkill").asButton;
            if (ChangeSkill != null)  
            {  
                ChangeSkill.onClick.Add((_) =>  
                {  
                    ...
                    {  
                        ...
                        isPlayingForward2 = !isPlayingForward2;  
                        }  
                    }  
                });  
            }  
            ...
            GComponent DBtnsWindow = DFunBtms.GetChild("DBtnsWindow").asCom;
            GComponent tmp = TMP.GetChild("BottomUI").asCom;
            GComponent DMinMapUI = TMP.GetChild("DMinMapUI").asCom;
            DMap = DMinMapUI.GetChild("DMap").asCom;
            
            DChatWindow = tmp.GetChild("DChatWindow").asList;
            bagItemCell = tmp.GetChild("bagItemCell").asList; 
            
             DChatWindow.itemRenderer = RenderListItem;
            GComponent DLevel = tmp.GetChild("DLevel").asCom;

            //任务切换
            DA2ESimpleButton1 = TaskAndRanksPanel.GetChild("DA2ESimpleButton1").asButton;
            if (DA2ESimpleButton1 != null)  
            {  
                DA2ESimpleButton1.onClick.Add((_) =>  
                {  
                    if (!isAnimationPlaying1)  
                    {  
                        isAnimationPlaying1 = true;  
                        DA2ESimpleButton1.enabled = false; // 禁用按钮  
                        Transition trans = TaskAndRanksPanel.GetTransition("go");  
                        ...
                        {  
                            ...
                            {  
                                ...
                                DA2ESimpleButton1.enabled = true;  
                            };  
  
                        if (isPlayingForward1)  
                        {  
                            trans.Play(onComplete);  
                        ...
                        {  
                            // 假设PlayReverse也可以接受回调，如果不是，需要另外处理  
                            trans.PlayReverse(onComplete);  
 
                        }  
  
                        isPlayingForward1 = !isPlayingForward1;  
                        }  
                    }  
                });  
            }  
            ...
            //商城按钮
            DBtnShop = DBtnsWindow.GetChild("DBtnShop").asButton;
            //切换UI攻击
            DBtnShow = DFunBtms.GetChild("DBtnShow").asButton;
           
           if (DBtnShow != null)  
            {  
                DBtnShow.onClick.Add((_) =>  
                {  
                    if (!isAnimationPlaying)  
                    {  
                        isAnimationPlaying = true;  
                        DBtnShow.enabled = false; // 禁用按钮  
                        Transition transGO = DArrackModelUI.GetTransition("GO");  
                        Transition trans = DFunBtms.GetTransition("come");  
                        if (trans != null)  
                        {  
                            PlayCompleteCallback onComplete = () =>  
                            {  
            ...
            {  
                UnityEngine.Debug.LogError("DBtnShow is null");  
            }
            //经验条
            DExpBar = tmp.GetChild("DExpBar").asProgress;
            //负重
            DWetBar = tmp.GetChild("DWetBar").asProgress;
            //等级
            DLevelText = DLevel.GetChild("DLevelText").asTextField;
            //血条
             HpWin1 = tmp.GetChild("DHPBall").asProgress;
             Hptitle = HpWin1.GetChild("title").asTextField;
             MpWin1 = tmp.GetChild("DMPBall").asProgress;
              Mptitle = MpWin1.GetChild("title").asTextField;
            //角色面板按钮
            DBtnState = DFunBtms.GetChild("DBtnState").asButton;
            //背包
            DBtnBag = DFunBtms.GetChild("DBtnBag").asButton;
            //单遥感
            GComponent DJoystick = TMP.GetChild("DJoystick").asCom;
            DJoystick.visible =false;
            GRoot.inst.AddChild(DJoystick);
            //双摇杆
            GComponent DoubleDJoystick = TMP.GetChild("DoubleDJoystick").asCom;
            DoubleDJoystick.visible =true;
            GRoot.inst.AddChild(DoubleDJoystick);
            //跑步
            rockingBarRun = new RockingBar();
            rockingBarRun.SetRun(DoubleDJoystick);
            rockingBarRun.onMove.Add(RockingBarMoveRun);
            rockingBarRun.onEnd.Add(RockingBarMoveRunEnd);
             //走路
            rockingBarWalk = new RockingBar();
            rockingBarWalk.SetWalk(DJoystick);
            rockingBarWalk.onMove.Add(RockingBarMoveWalk);
            rockingBarWalk.onEnd.Add(RockingBarMoveWalkEnd);
            //切换遥感
            DRun = tmp.GetChild("DRun").asLoader;
            TMP.SetSize(Settings.ScreenWidth,Settings.ScreenHeight);
            GRoot.inst.SetContentScaleFactor(Settings.ScreenWidth,Settings.ScreenHeight,ScreenMatchMode.MatchHeight);
            TMP.x = (GRoot.inst.width - TMP.width)/2;
            TMP.y = GRoot.inst.height - TMP.height;
            TMP.visible = true;
            GRoot.inst.AddChild(TMP);
            DRun.onClick.Add(() =>
            {
                SoundManager.PlaySound(SoundList.ButtonB);
                if (IsDRun)
                {
                    DJoystick.visible = true;
                    DoubleDJoystick.visible = false;
                    IsDRun = false;
                    DRun.url = "ui://jk0io98jldxan23z";
                }
                else
                {
                    DJoystick.visible = false;
        ...
        //跑步
        private void RockingBarMoveRun(EventContext context)
        {
            if (JoystickMovetime > CMain.Time) return;
            mirDirection = (MirDirection)context.data;
            if(!isMoveRocking)
            {
                JoystickMovetime = CMain.Time + 30;
            }
            else
                MapControl.MapButtons = MouseButtons.Right;
                isMoveRocking = true;
                ActiveControl =MapControl;
                MapObject.TargetObject = null;
                MapObject.TargetObjectID = 0;
                MapControl.AutoPath = false;
                MapControl.AutoHit = false;
                MapControl.AutoRun = false;

        }
        //停止跑步
        private void RockingBarMoveRunEnd()
        {
            isMoveRocking = false;
            MapControl.MapButtons = MouseButtons.None;
            mirDirection = MirDirection.None;
            JoystickMovetime = CMain.Time +100;
        }
        //走路
        private void RockingBarMoveWalk(EventContext context)
        {
            if(JoystickMovetime > CMain.Time)return;
            mirDirection = (MirDirection)context.data;
            if(!isMoveRocking)
            {
                ...
                MapControl.MapButtons = MouseButtons.Left;
                isMoveRocking = true;
                ActiveControl = MapControl;
                MapObject.TargetObject = null;
                MapObject.TargetObjectID = 0;
        ...
        //停止走路
        private void RockingBarMoveWalkEnd()
        {
            isMoveRocking = false;
            MapControl.MapButtons = MouseButtons.None;
            mirDirection = MirDirection.None;
    ...
    public sealed class MapControl : MirControl
    {
        ...
        private UnityEngine.RenderTexture nightTexture;
        private UnityEngine.RenderTexture cellTexture;

        private static UnityEngine.RenderTexture lastBaseTexture = null;
        private static UnityEngine.RenderTexture lastOverlayTexture = null;
        private static UnityEngine.RenderTexture lastResultTexture = null;
        public static UserObject User
        {
            get { return MapObject.User; }
            set { MapObject.User = value; }
        ...
        private void DrawFloor()
        {
            ...
            if (cellTexture == null)
            {
                cellTexture = new UnityEngine.RenderTexture(Settings.ScreenWidth, Settings.ScreenHeight, 0, UnityEngine.RenderTextureFormat.ARGB32)
                {
                    filterMode = UnityEngine.FilterMode.Bilinear
                };
                cellTexture.Create();
            }
            UnityEngine.Graphics.SetRenderTarget(cellTexture);
            UnityEngine.GL.Clear(true, true, new UnityEngine.Color(0,0,0,1));
            UnityEngine.Graphics.SetRenderTarget(null);
            UnityEngine.Graphics.DrawTexture(new UnityEngine.Rect(0, 0, Settings.ScreenWidth, Settings.ScreenHeight), cellTexture);
        ...
        private void DrawObjects()
        {
              ...
              if (Settings.HighlightTarget)
            {
                if (MapObject.MouseObject != null && !MapObject.MouseObject.Dead && MapObject.MouseObject != MapObject.TargetObject && MapObject.MouseObject.Blend)
                    MapObject.MouseObject.DrawHightLightBlend();

                if (MapObject.TargetObject != null)
                    MapObject.TargetObject.DrawHightLightBlend();
            }
        ...
        private Color GetBlindLight(Color light)
        {
            ...
            light = Color.FromArgb(255, (byte)Math.Max(20, light.R - (count * 10)), (byte)Math.Max(20, light.G - (count * 10)), (byte)Math.Max(20, light.B - (count * 10)));
 ...
 #region 黑夜环境及光照纹理
        #region 配置并生成黑夜环境
        private void DrawAmbientLight(LightSetting setting)
        {
            // 检查是否是黑夜，不是则退出
            ...
            {
                nightTexture = new UnityEngine.RenderTexture(Settings.ScreenWidth, Settings.ScreenHeight, 0, UnityEngine.RenderTextureFormat.ARGB32)
                {
                    filterMode = UnityEngine.FilterMode.Bilinear
                };
                nightTexture.Create();
            ...
            //设置当前的渲染目标为黑暗环境
            UnityEngine.Graphics.SetRenderTarget(nightTexture);

            //使用暗黑颜色填充暗黑环境
            UnityEngine.GL.Clear(true, true, new UnityEngine.Color(darkness.R / 255f, darkness.G / 255f, darkness.B / 255f, darkness.A / 255f * backgroundOpacity));
            #endregion

            #region 遍历光源对象渲染光照
            //启动混合，配置混合参数，准备绘制光源
            DXManager.SetLightsBlend(true, backgroundOpacity, BlendMode.LIGHT, darkness); // 启用混合效果，以便将光源纹理和黑夜环境效果正确叠加

            //绘制光源
            DrawLightSources(); // 该方法将处理所有光源并将其绘制到lightTexture

            //结束混合模式
            DXManager.SetLightsBlend(false,1f,BlendMode.NORMAL,Color.White); // 关闭混合效果
            #endregion
            ...
            //混合最终结果
            UnityEngine.Material material = new UnityEngine.Material(UnityEngine.Shader.Find("Shader/NightBlend"));
            material.SetTexture("_MainTex", nightTexture);

            // 设置光线亮度
            float brightnessValue = 1.25f;
            material.SetFloat("_Brightness", brightnessValue);

            // 绘制到屏幕
            UnityEngine.Graphics.DrawTexture(new UnityEngine.Rect(0, 0, Settings.ScreenWidth, Settings.ScreenHeight), nightTexture, material);
        }
        ...
        private void DrawLightSources()
        {
            ...
            foreach (var ob in Objects)
            {
                if (ob.Light > 0 && (!ob.Dead || ob == MapObject.User || ob.Race == ObjectType.Spell))
                {
                    ...
                    lightRange = UnityEngine.Mathf.Clamp(lightRange, 0, DXManager.Lights.Count - 1);
                    p = ob.DrawLocation;

                    Color lightColour = ob.LightColour; // 获取骑乘特效
                    if (ob.Race == ObjectType.Player)
                    {
                        switch (ob.Light / 15)
                        {
                    ...
                    // 绘制光源
                    if (DXManager.Lights[lightRange] != null)
                    {
                        p.Offset(-(DXManager.LightSizes[lightRange].X / 2) - (CellWidth / 2)+8, -(DXManager.LightSizes[lightRange].Y / 2) - (CellHeight / 2) - 5);
                ...
                foreach (var effect in ob.Effects)
                {
                    ...
                    Color lightColour = effect.LightColour;
                    ...
                    if (DXManager.Lights[light] != null)
                    {
            ...
            for (int y = MapObject.User.Movement.Y - ViewRangeY - 24; y <= MapObject.User.Movement.Y + ViewRangeY + 24; y++)
            {
                ...
                for (int x = MapObject.User.Movement.X - ViewRangeX - 24; x < MapObject.User.Movement.X + ViewRangeX + 24; x++)
                {
                    ...
                    if (cellInfo.Light <= 0 || cellInfo.Light >= 10) continue;

                    Color lightIntensity;
                    light = (cellInfo.Light % 10) * 3;

                    // 确定光源亮度
                    switch (cellInfo.Light / 10)
                    {
                        case 1:
                    ...
                    int imageIndex = (cellInfo.FrontImage & 0x7FFF) - 1;
                    if (cellInfo.FrontAnimationFrame > 0)
                    {
                        p.Offset(Libraries.MapLibs[cellInfo.FrontIndex].GetOffSet(imageIndex));
                    }

                    light = UnityEngine.Mathf.Clamp(light, 0, DXManager.Lights.Count - 1);
        ...
        //     // if (DXManager.LightTexture == null)
        //     // {
        //     //     // 创建 Texture2D，大小为屏幕宽度和高度
        //     //     DXManager.LightTexture = new UnityEngine.Texture2D(Settings.ScreenWidth, Settings.ScreenHeight);
        //     // }
        ...
        //     if (!DXManager.LightTextures.ContainsKey(darkness.ToHex()))
        //     {
        //         DXManager.LightTextures[darkness.ToHex()] = new UnityEngine.Texture2D(Settings.ScreenWidth, Settings.ScreenHeight);
        //         UnityEngine.Color[] pixels = DXManager.LightTextures[darkness.ToHex()].GetPixels();
        //         for (int i = 0; i < pixels.Length; i++)
        //         {
        //             pixels[i] = new UnityEngine.Color(darkness.R / 255, darkness.G / 255, darkness.B / 255, darkness.A / 255);
        //         }
        //         DXManager.LightTextures[darkness.ToHex()].SetPixels(pixels);
        //         DXManager.LightTextures[darkness.ToHex()].Apply();// 应用更改
        //     }
        ...
        //     DXManager.SetBlend1(true);
        //      DXManager.Draw(DXManager.LightTextures[darkness.ToHex()], new Rectangle(0, 0, Settings.ScreenWidth, Settings.ScreenHeight), Vector3.Zero, Color.White);
        //     // DXManager.SetRenderState(UnityEngine.Rendering.BlendMode.SrcAlpha, UnityEngine.Rendering.BlendMode.One);
        ...
        //     DXManager.SetBlend1(false);
        //     // DXManager.SetSurface(oldSurface);

        //     // DXManager.SetRenderState(UnityEngine.Rendering.BlendMode.Zero, UnityEngine.Rendering.BlendMode.SrcColor);
        ...
        private void CheckInput()
        {
            ...
            if (MouseControl == this || ((GameScene)(MirScene.ActiveScene)).isMoveRocking)
            {
        ...
        {
            if(((GameScene)(MirScene.ActiveScene)).isMoveRocking)
            {
                return((GameScene)(MirScene.ActiveScene)).mirDirection;
            }
            else
            {
                return PCMouseDirection(ratio);
            }
        }
        public static MirDirection PCMouseDirection(float ratio = 45F) //22.5 = 16
        {
