...
namespace Client.MirObjects
{
    public class PlayerObject : MapObject
    {
        ...
        public override void Process()
        {
            ...
            if (this == User)
            {
                if (CMain.Time - GameScene.LastRunTime > 899) GameScene.CanRun = false;
            }
            ...
            switch (CurrentAction)
            {
                ...
                case MirAction.DashAttack:
                    ...
                    #region 根据角色的动作定义角色移动的距离
                    var i = 0;
                    if (CurrentAction == MirAction.MountRunning) i = 3;
                    else if (CurrentAction == MirAction.Running) i = (Sprint && !Sneaking ? 3 : 2);
                    else i = 1;
                    ...
                    double sum = FrameInterval * Frame.Count;

                    switch (Direction)
                    {
                        case MirDirection.Up:
                            if (Settings.SmoothDraw)
                            {
                                OffSetMove = new Point(
                                0,
                                MapControl.CellHeight * i - (int)((MapControl.CellHeight * i / (double)(sum)) * (sum - Math.Min((CMain.Time - GameScene.LastRunTime), sum)))
                                );
                                break;
                            }
                            else
                        ...
                        case MirDirection.UpRight:
                            ...
                            {
                                OffSetMove = new Point(
                                -MapControl.CellWidth * i + (int)((MapControl.CellWidth * i / (double)(sum)) * (sum - Math.Min((CMain.Time - GameScene.LastRunTime), sum))),
                                MapControl.CellHeight * i - (int)((MapControl.CellHeight * i / (double)(sum)) * (sum - Math.Min((CMain.Time - GameScene.LastRunTime), sum)))
                                );
                                break;
                            }
            ...
            #region 每一帧中更新人物特效
            for (int i = 0; i < Effects.Count; i++) Effects[i].Process();
            #endregion
        ...
        {
            //检查 NextAction 是否不为 null 并且当前游戏场景是否可以移动。如果 CanMove 为 false，则表示角色不能执行移动相关的动作
            if (NextAction != null && !GameScene.CanMove)
            {
                //根据 NextAction 的动作类型进行分支判断。如果下一动作是走、跑、骑马走、骑马跑、被推、冲刺、跳跃等动作之一，则直接返回，不设置新动作。
            ...
            //检查是否是用户控制的角色，并且当前时间小于下一个动作时间。如果是，则继续。
            if (User == this && CMain.Time < MapControl.NextAction)// && CanSetAction)
            {
                //NextMagic = null;
                return;
            }
            //检查 ActionFeed 列表是否为空，表示没有排队的动作
            if (ActionFeed.Count == 0)
            {
                //设置当前动作为静止状态
                CurrentAction = MirAction.Standing;
                //如果当前时间大于 BlizzardStopTime，则保持当前动作；否则设置为姿态2。
                ...
                //判断如果当前动作是静止，接下来的动作设置依赖于类的类型
                if (CurrentAction == MirAction.Standing)
                {
                    //如果角色是弓箭手且拥有相应的武器，保持当前动作为站立
                    if (Class == MirClass.Archer && HasClassWeapon) CurrentAction = MirAction.Standing;
                    //如果不是弓箭手，根据时间选择是保持站立，还是变为某种姿态
                    else CurrentAction = CMain.Time > StanceTime ? MirAction.Standing : MirAction.Stance;
                }
                //如果角色在钓鱼，将动作设置为钓鱼等待状态
                ...
                FrameIndex = 0;
                EffectFrameIndex = 0;
                //如果当前的地图位置不同于角色的位置，进行位置更新
                if (MapLocation != CurrentLocation)
                {
                    //从地图中移除角色，并更新位置后再添加回去，以便刷新显示
                    GameScene.Scene.MapControl.RemoveObject(this);
                    MapLocation = CurrentLocation;
                    GameScene.Scene.MapControl.AddObject(this);
                }
                //如果帧没有找到，则直接返回，不执行后续操作
                if (Frame == null) return;
                //设置帧动画间隔和特效间隔
                FrameInterval = Frame.Interval;
                EffectFrameInterval = Frame.EffectInterval;
                //调用 SetLibraries 方法，更新动画库
                SetLibraries();
            }
            else
            {
                //取出排队动作 ActionFeed 中的第一个动作存入 action，并将该动作从列表中删除
                QueuedAction action = ActionFeed[0];
                ActionFeed.RemoveAt(0);
                //将当前动作设置为队列中的动作
                CurrentAction = action.Action;
                //检查角色是否在骑马状态，如果是，进入相应的处理
                if (RidingMount)
                {
                    switch (CurrentAction)
                    {
                ...
                //设置当前角色的位置为队列中动作所指定的位置
                CurrentLocation = action.Location;
                //保存当前方向，然后将方向更新为动作队列中的方向
                ...
                //根据动作的位置更新 temp，确保角色 Y 坐标不小于当前 Y 坐标
                temp = new Point(action.Location.X, temp.Y > CurrentLocation.Y ? temp.Y : CurrentLocation.Y);
                //检查当前地图位置是否和目标位置不同
                if (MapLocation != temp)
                {
                    //如果不同，则从地图中移除角色，更新位置，再重新添加回地图中
                    GameScene.Scene.MapControl.RemoveObject(this);
                    MapLocation = temp;
                    GameScene.Scene.MapControl.AddObject(this);
                }

                //声明一个布尔变量，用于表示弓箭手是否正在布置陷阱
                        ...
                        switch (Spell)
                        {
                            case Spell.ShoulderDash:
                                ...
                                if (this == User)
                                {
                                    ...
                                    Client.MirNetwork.Network.Enqueue(new C.Magic { ObjectID = GameScene.User.ObjectID, Spell = Spell, Direction = Direction, });
                                }
                                break;
                            ...
                            case Spell.FlashDash:
                                {
                                    ...
                                    if (this == User)
                                    {
                                        ...
                                        if (JumpDistance != 0) Client.MirNetwork.Network.Enqueue(new C.Magic { Spell = Spell, Direction = Direction });
                                    }
                                }
                                break;
                            ...
                            case Spell.ExplosiveTrap:
                                ...
                                if (this == User)
                                {
                                    ...
                                    Client.MirNetwork.Network.Enqueue(new C.Magic { ObjectID = GameScene.User.ObjectID, Spell = Spell, Direction = Direction, TargetID = targetID, Location = location });
                                    MapControl.NextAction = CMain.Time + 1000;
                                    GameScene.SpellTime = CMain.Time + 1500; //Spell Delay
                                }
                                break;
                ...
                if (this == User)
                {
                    switch (CurrentAction)
                    {
                        ...
                        case MirAction.MountStanding:
                            Client.MirNetwork.Network.Enqueue(new C.Turn { Direction = Direction });
                            MapControl.NextAction = CMain.Time + 2500;
                        ...
                        case MirAction.Sneek:
                            ...
                            Client.MirNetwork.Network.Enqueue(new C.Walk { Direction = Direction });
                        ...
                        case MirAction.MountRunning:
                            GameScene.LastRunTime = CMain.Time;
                            Client.MirNetwork.Network.Enqueue(new C.Run { Direction = Direction });
                            GameScene.Scene.MapControl.FloorValid = false;
                        ...
                        case MirAction.DashAttack:
                            ...
                            break;
                        case MirAction.Mine:
                            Client.MirNetwork.Network.Enqueue(new C.Attack { Direction = Direction, Spell = Spell.None });
                        ...
                        case MirAction.MountAttack:
                            ...
                            Client.MirNetwork.Network.Enqueue(new C.Attack { Direction = Direction, Spell = Spell });
                        ...
                        case MirAction.AttackRange1:
                            {
                                ...
                                Client.MirNetwork.Network.Enqueue(new C.RangeAttack { Direction = Direction, Location = CurrentLocation, TargetID = targetID, TargetLocation = location });
                            }
                            break;
                        case MirAction.AttackRange2:
                        case MirAction.Spell:
                        ...
                        case MirAction.Harvest:
                            ...
                            {
                                Client.MirNetwork.Network.Enqueue(new C.Harvest { Direction = Direction });
                                MapControl.NextAction = CMain.Time + 2500;
                            }
                            break;
        ...
        {
            return MapControl.MapLocation == CurrentLocation || BodyLibrary != null && BodyLibrary.VisiblePixel(DrawFrame + ArmourOffSet, p,FinalDrawLocation);
        }
