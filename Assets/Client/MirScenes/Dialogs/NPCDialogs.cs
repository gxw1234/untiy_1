...
namespace Client.MirScenes.Dialogs
{
    public sealed class NPCDialog : MirImageControl
    {
        ...
        Font font = new Font(Settings.FontName, 12F);
        ...
        public void NewText(List<string> lines, bool resetIndex = true)
        {
            ...
            for (int i = _index; i < lastLine; i++)
            {
                ...
                foreach (Match match in matchList.OrderBy(o => o.Index).ToList())
                {
                    ...
                    Size size = CMain.GetTextSize(text,  TextLabel[i].Size, TextLabel[i].Font);
                    UnityEngine.Vector2 textSize = CMain.style.CalcSize(new UnityEngine.GUIContent(text));

                    if (R.Match(match.Value).Success)
                        NewButton(txt, action, TextLabel[i].Location.Add(new Point(size.Width - 9, 0)));
